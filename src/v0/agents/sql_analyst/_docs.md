...
1. Based on the user's location preferences the report will need to target:
  - Entire Country
  - region
  - state_name
  - cbsa_name
...
We need to add the `region` column to the uszips database and put use the following US Regions:
...
The SQL agent will have this query parametrized and will be able to create the data necessary for pie charts, bar charts, histograms, etc.